...
namespace RSDKv5
{
    ...
    {
        static readonly ConcurrentDictionary<int, string> hashStringCache = new();
        ...
        public bool usingHash => name == null;

        readonly string hashString;

        public NameIdentifier(string name)
        {
            var nameHashCode = name.GetHashCode();

            var hashBytes = MD5Hasher.GetHash(name);
            hash = hashBytes;
            if (hashStringCache.TryGetValue(nameHashCode, out var cachedHashString))
            {
                hashString = cachedHashString;
            }
            else
            {
                hashString = BitConverter.ToString(hashBytes).Replace("-", string.Empty).ToLower();
                hashStringCache[nameHashCode] = hashString;
            }
        ...
        {
            if (obj is not NameIdentifier ni)
            {
                return false;
            }
            return ni.hashString == this.hashString;
        }
    }
    #endregion
    ...
    public class VariableValue
    {
        ...
        {
            get { checkType(VariableTypes.bool32); return value_bool; }
            set { checkType(VariableTypes.bool32); value_bool = value; }
        }
